classdef Vertical
    ...
    methods
        % Constructor
        function obj = Vertical(vb, immersion, cm)
            % Initialize the angle of attack and calculate the lift and drag coefficients
            global ro_water;
            obj.vb = vb;
            obj.thetaL = 0;
            obj.immersion = immersion;
            obj.cm = cm;
            obj.S = obj.cm * obj.immersion;
            obj.AR = obj.immersion^2 / obj.S;
            [cl_rf, cd_rf] = Coefficent_f3(obj.thetaL);  % Assuming Coefficent_f3 calculates cl and cd based on thetaL
            obj.Cl = cl_rf / (1 + 2 / obj.AR); % 3D lift coefficient
            obj.Cd = cd_rf;  % Assign drag coefficient
            obj.Cdw = 0.025 * obj.Cl^2 * obj.cm / obj.immersion; % Wave drag coefficient
        ...
        function Torque = Torque(obj)
            ...
            Torque = - Drag * abs(obj.z + obj.immersion/2);
        end

        function  DisplayGeometry(obj)  
             disp('-----------------------------------')
            disp(['Span: ' num2str(obj.immersion)]);
