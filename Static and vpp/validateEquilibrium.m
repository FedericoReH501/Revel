function isEquilibrium = validateEquilibrium(sol)
...
wind = Wind(sol(1),wind_speed,60); % initialize speed[Kn] and Angle[deg]
boat = Boat(wind); % pass the wind to our boat model
crew = Crew(75,[0.3,2]); % define the crew mass[kg] , and range of movemnt

centerVertical = Vertical(sol(1), 0.4, 0.12);
rudderVertical = Vertical(sol(1), 0.3, 0.12);

centerFoil = CenterFoil(sol(1), sol(2), sol(4) , 0.12); % initialize center foil model passing AoA[degree] , span & chord[m]
rudderFoil = RudderFoil(sol(1), sol(2) , sol(5), 0.075); % rudder foil model passing span[m]
